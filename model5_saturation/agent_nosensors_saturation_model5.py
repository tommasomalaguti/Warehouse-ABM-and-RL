...
class Forklift2(Agent):
    def __init__(self, unique_id, model, empty_speed, loaded_speed, load_time, unload_time, searching_speed, color, visited_color):
        ...
        self.purple_waitpoints = []  # List to track purple waitpoints in order of appearance
        self.step_count = 0  # Initialize step counter for all steps
    ...
    def move(self):
        if self.path:
            ...
            # After moving, update the color of the previous waitpoint to the visited color
            if not self.currently_loaded and self.previous_waitpoint and self.previous_waitpoint != self.pos:
                for agent in self.model.grid.get_cell_list_contents(self.previous_waitpoint):
                    if isinstance(agent, WaitPoint) and agent.color != "purple":
            ...
            if self.pos == self.target:
                if self.currently_loaded:
                    ...
                    # After unloading, turn the first purple waitpoint red and follow its path
                    self.reset_red_waitpoint()
                    self.color_next_purple_waitpoint_red()
                else:
    ...
    def step(self):
        ...
        if self.step_count % 10 == 0:
            self.color_random_waitpoint_purple()
    ...
    def get_zone_of_red_waitpoint(self):
        for agent in self.model.schedule.agents:
            if isinstance(agent, WaitPoint) and agent.color == "red":
                red_pos = agent.pos
                if red_pos in self.zone_1_coords:
                    return 1
                elif red_pos in self.zone_2_coords:
                    return 2
                elif red_pos in self.zone_3_coords:
                    return 3
                elif red_pos in self.zone_4_coords:
                    return 4
        return None

    def set_picking_target(self):
        zone = self.get_zone_of_red_waitpoint()
        if zone == 1:
    ...
    def color_random_waitpoint_purple(self):
        waitpoints = [agent for agent in self.model.schedule.agents if isinstance(agent, WaitPoint) and agent.color == "yellow"]
        if waitpoints:
            random_waitpoint = self.random.choice(waitpoints)
            random_waitpoint.color = "purple"
            self.purple_waitpoints.append(random_waitpoint)  # Track the purple waitpoints in order
            if not any(agent.color == "red" for agent in self.model.schedule.agents if isinstance(agent, WaitPoint)):
                self.color_next_purple_waitpoint_red()
        else:
            self.model.no_more_waitpoints = True  # Indicate no more waitpoints available

    def reset_red_waitpoint(self):
        red_waitpoints = [agent for agent in self.model.schedule.agents if isinstance(agent, WaitPoint) and agent.color == "red"]
        if red_waitpoints:
            red_waitpoint = red_waitpoints[0]
            red_waitpoint.color = "yellow"

    def reset_visited_waitpoints(self):
        for agent in self.visited_waitpoints:
            agent.seen_by[self.unique_id] = "yellow"
        self.visited_waitpoints = []

    def color_next_purple_waitpoint_red(self):
        if self.purple_waitpoints:
            next_purple = self.purple_waitpoints.pop(0)  # Get the first purple waitpoint in the list
            next_purple.color = "red"
            # Set the new red waitpoint's zone path
            self.set_picking_target()
