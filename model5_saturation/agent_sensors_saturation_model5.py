...
class Forklift(Agent):
    ...
    def move(self):
        if self.path:
            ...
            # Check if 20 steps have been taken
            if self.steps_taken % 10 == 0:
                self.color_random_waitpoint_purple()

            # Calculate time and energy consumption based on whether the forklift is loaded or not
            if self.currently_loaded:
                time_increment = 1 / self.speed_loaded
            else:
                time_increment = 1 / self.speed_empty
