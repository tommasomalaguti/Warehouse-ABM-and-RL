...
from agent_nosensors_saturation_model5 import Forklift2, NormalWall, StorageWall, StorageZone, WaitPoint
import numpy as np
...
class WarehouseModel2(Model):
    def __init__(self, width, height, total_time_hours=None, max_cycles=None, steps_taken=None):
        super().__init__()
        ...
        self.no_more_waitpoints = False  # Flag to track if there are no more waitpoints
        self.max_time_seconds = total_time_hours * 3600 if total_time_hours is not None else None
        self.max_cycles = max_cycles
        self.steps_taken = steps_taken  # Initialize steps_taken
        self.current_step_count = 0  # Counter for steps taken
    ...
    def draw_warehouse(self):
        ...
        for x in range(18, 30):
            wall = NormalWall((x, 11), self)
            self.schedule.add(wall)
            self.grid.place_agent(wall, (x, 11))
        # boundary horizontal wall low right - low
        for x in range(18, 30):
            wall = NormalWall((x, 0), self)
            self.schedule.add(wall)
            self.grid.place_agent(wall, (x, 0))
    ...
    def reset_loading_slot(self):
        # Reset the current red cell to black
    ...
    def step(self):
        ...
        # Stop the simulation if there are no more waitpoints to turn purple
        if self.no_more_waitpoints:
            print("No more waitpoints available to turn purple. Stopping simulation.")
            self.running = False
