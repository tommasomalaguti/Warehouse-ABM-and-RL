...
# Load your datasets
data_fork1 = pd.read_csv("/Users/tommasomalaguti/Documents/Python/Tesi/model5_twoforks/simulation_results_forklift_1_model5_twoforks.csv")
data_fork2 = pd.read_csv("/Users/tommasomalaguti/Documents/Python/Tesi/model5_twoforks/simulation_results_forklift_2_model5_twoforks.csv")

# Check if all datasets have the same columns
assert all(data_fork1.columns == data_fork2.columns), "Column mismatch among datasets"

# Combine the datasets
combined_data = data_fork1.copy()

# Sum the relevant columns across all forklifts
combined_data['Unloading Cycles'] += data_fork2['Unloading Cycles']
combined_data['Energy Consumption'] += data_fork2['Energy Consumption']
combined_data['Total Distance'] += data_fork2['Total Distance']

# Take the maximum time from the three datasets
combined_data['Total Time'] = combined_data[['Total Time']].join(data_fork2[['Total Time']], rsuffix='_fork2').max(axis=1)
...
summary_data['Driver Salary Cost (€)'] = (summary_data['Total Time'] / 3600) * driver_salary_per_hour * 2
