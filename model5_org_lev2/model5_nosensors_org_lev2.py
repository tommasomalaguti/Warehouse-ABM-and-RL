...
from agent_nosensors_org_lev2_model5 import Forklift2, NormalWall, StorageWall, StorageZone, WaitPoint
import numpy as np
...
class WarehouseModel2(Model):
    ...
    def reset_loading_slot(self):
        # Reset the current red cell to yellow or grey
        for agent in self.schedule.agents:
            if isinstance(agent, WaitPoint) and agent.color == "red":
                agent.color = "yellow" if agent.zone in [1, 2, 3, 4] else "grey"
                break
