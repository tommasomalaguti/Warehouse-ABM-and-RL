import argparse
import pandas as pd
from model5_nosensors_threeforks import WarehouseModel2
...
def main(total_time_hours=None, max_cycles=None):
    ...
    data.to_csv("simulation_results_nosensors_montecarlos_model5_threeforks.csv")
    ...
    for i, forklift_agent in enumerate(model.forklifts, start=1):
        ...
        forklift_data.to_csv(f"simulation_results_forklift_{i}_model5_threeforks.csv")
        ...
        speeds_data.to_csv(f"speeds_used_forklift_{i}_model5_threeforks.csv", index=False)
        ...
        searching_speeds_data.to_csv(f"searching_speeds_used_forklift_{i}_model5_threeforks.csv", index=False)
