...
from test2 import ForkliftEnv  # Replace 'test2' with the actual filename where ForkliftEnv is defined
...
model = PPO(
    ...
    ent_coef=0.09,       # Coefficient for the entropy term, promoting exploration
)
...
# Define a custom callback to track rewards, energy consumption, and handle accident occurrences
class RewardTrackingCallback(BaseCallback):
    def __init__(self, verbose=0):
        super(RewardTrackingCallback, self).__init__(verbose)
        self.episode_rewards = []
        self.global_energy = []
        self.accident_count = 0  # To track how many times an accident occurred

    def _on_step(self) -> bool:
        # Check if the episode is done
        done = self.locals['dones'][0]
        info = self.locals['infos'][0]  # Access the info dictionary
        if done:
            env = self.locals['env'].envs[0]
            self.episode_rewards.append(np.sum(self.locals['rewards']))
            self.global_energy.append(env.global_energy_consumption)
        
        if info.get('accident_occurred') == "Yes":
            self.accident_count += 1
            accident_severity = info['accident_prob']
            # Apply a penalty proportional to the accident severity
            reward = self.locals['rewards'][0] - (accident_severity * 10)  # Adjust the multiplier as needed
            self.locals['rewards'][0] = reward
