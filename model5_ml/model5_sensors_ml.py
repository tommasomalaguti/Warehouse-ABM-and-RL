...
from agent_sensors_ml import Forklift, NormalWall, StorageWall, StorageZone, WaitPoint
import numpy as np
...
class WarehouseModel(Model):
    def __init__(self, width, height, total_time_hours=None, max_cycles=None):
        ...
        self.generate_monte_carlo_samples()  # Generate samples once at the start

        self.reset()  # Initialize the model

    def reset(self):
        # Reset the grid and schedule
        self.grid = MultiGrid(self.width, self.height, False)  # Disable toroidal (wrap-around) grid
        ...
        # Create a forklift and place it at a specific position
        ...
        self.max_time_seconds = self.total_time_hours * 3600 if self.total_time_hours is not None else None

        # Draw warehouse layout
        self.draw_warehouse()

        # Randomize the loading slot
        self.randomize_loading_slot()

        # Mark the model as running
        self.running = True
    ...
    def generate_monte_carlo_samples(self):
        ...
        # Define the lower bound for the truncated normal distribution
        lower_bound = 10

        # Calculate the bounds for the truncated normal distribution
        a_load, b_load = (lower_bound - t_load_mean) / sigma_load, np.inf
        a_unload, b_unload = (lower_bound - t_unload_mean) / sigma_unload, np.inf

        # Generate 1,000,000 random samples for each factor
        ...
        self.load_times = truncnorm(a_load, b_load, loc=t_load_mean, scale=sigma_load).rvs(1000000)
        self.unload_times = truncnorm(a_unload, b_unload, loc=t_unload_mean, scale=sigma_unload).rvs(1000000)
