...
from test_curr import ForkliftEnv  # Replace 'test' with the actual filename where ForkliftEnv is defined

# Create environment
env = ForkliftEnv()

# Define a PPO model with some exploration-focused hyperparameters
model = PPO(
    'MlpPolicy', 
    env, 
    verbose=1, 
    learning_rate=3e-4,  # Lower learning rate for more stable learning
...
class RewardTrackingCallback(BaseCallback):
    ...
    def _on_training_end(self) -> None:
        global episode_rewards, episode_energy_consumption, global_energy_consumption
