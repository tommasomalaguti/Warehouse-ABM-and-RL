...
class ForkliftEnv(gym.Env):
    def __init__(self, alpha=2):
        ...
        self.loaded_speed_values = [1, 1.2, 1.4, 1.6, 1.8, 2, 2.2, 2.4, 2.6, 2.8, 3]
        self.empty_speed_values = [3, 3.2, 3.4, 3.6, 3.8, 4, 4.2, 4.4, 4.6, 4.8, 5]
        ...
        # Fix to account for action space
        self.n_actions = len(self.loaded_speed_values) * len(self.empty_speed_values) * len(self.loading_time_values) * len(self.unloading_time_values)
        ...
        self.csv_file = open('forklift_training_sim_results_empty.csv', mode='w', newline='')
        self.csv_writer = csv.writer(self.csv_file)
        self.csv_writer.writerow([
            'Episode', 'Step', 'Task', 'Loaded Speed', 'Empty Speed', 'Loading Time', 'Unloading Time', 
    ...
    def reset(self):
        ...
        self.original_distance_to_target = self.distance_to_target  # Store the initial distance to target
        ...
        self.travelling_empty_steps = 0  # Counter for steps while traveling empty
        self.step_count = 0
    ...
    def step(self, action):
        loaded_speed, empty_speed, loading_time, unloading_time = self.decode_action(action)
        done = False
        ...
        if self.current_task == 0:  # Traveling empty
            accident_prob = 0
            self.travelling_empty_steps += 1
            self.distance_to_target -= empty_speed
            time_increment = 1 / empty_speed
            ...
            reward = - self.alpha * energy_consumption 

            self.log_state("Traveling Empty", action, accident_prob, self.load_weight, done, random_value, accident_occurred, self.total_time, self.total_energy_consumption, self.cumulative_energy_per_episode, self.global_energy_consumption, reward)

            if not done and self.distance_to_target <= 0:
                self.distance_to_target = 0
                self.current_task = 1  # Move to loading

        elif self.current_task == 1:  # Loading
            ...
            # Reset the distance to target for the loaded journey
            self.distance_to_target = self.distance_to_target
            ...
            if not done:
                self.current_task = 2  # Move to traveling loaded
        
        elif self.current_task == 2:  # Traveling loaded
            ...
            if not done and self.distance_to_target <= 0:
                self.distance_to_target = 0
                self.current_task = 3  # Move to unloading

        elif self.current_task == 3:  # Unloading
    ...
    def decode_action(self, action):
        ...
        action //= len(self.unloading_time_values)
        loading_time_index = action % len(self.loading_time_values)
        action //= len(self.loading_time_values)
        empty_speed_index = action % len(self.empty_speed_values)
        action //= len(self.empty_speed_values)
        loaded_speed_index = action % len(self.loaded_speed_values)

        loaded_speed = self.loaded_speed_values[loaded_speed_index]
        empty_speed = self.empty_speed_values[empty_speed_index]
        loading_time = self.loading_time_values[loading_time_index]
        unloading_time = self.unloading_time_values[unloading_time_index]

        return loaded_speed, empty_speed, loading_time, unloading_time
    ...
    def log_state(self, task, action, accident_prob, load_weight, done, random_value, accident_occurred, total_time, total_energy_consumption, cumulative_energy_per_episode, global_energy_consumption, reward):
        loaded_speed, empty_speed, load_time, unload_time = self.decode_action(action)
        self.csv_writer.writerow([
            self.episode, self.step_count, task, loaded_speed, empty_speed, load_time, unload_time, 
    ...
    def accident_probability_loading(self, weight, load_time):
        ...
        normalized_loading_time = (load_time - 20) / (300 - 20)
        beta_weight = 3
        beta_loading_time = -5
        gamma = 0.007
    ...
    def accident_probability_unloading(self, weight, unload_time):
        normalized_weight = (weight - 500) / 2500
        normalized_unloading_time = (unload_time - 20) / (300 - 20)
        beta_weight = 3
